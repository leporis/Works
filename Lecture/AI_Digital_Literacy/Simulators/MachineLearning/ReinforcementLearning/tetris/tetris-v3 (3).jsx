...
const generateLearningReport = (agent, stats) => {
  ...
  const report = `# 🧠 Tetris DQN 학습 결과 보고서
...
AI가 학습을 통해 습득한 테트리스 전략의 중요도입니다:
...
- **구멍 회피**: 블록 사이에 빈 공간을 만들지 않습니다
- **높이 관리**: 블록을 가능한 낮게 쌓습니다
- **평탄화**: 표면을 평평하게 유지합니다
- **최대높이 제한**: 한쪽이 너무 높아지지 않게 합니다
...
### 학습 단계
${gen < 100 ? '🌱 초기 단계 - 무작위 탐색 중' :
  gen < 500 ? '🔍 탐색 단계 - 패턴 발견 중' :
  gen < 2000 ? '📚 학습 단계 - 전략 학습 중' :
  gen < 5000 ? '🎯 응용 단계 - 전략 최적화 중' :
  '🏆 숙련 단계 - 고급 플레이'}

### 성과 평가
${avgSuccess >= 80 ? '✅ AI가 테트리스의 핵심 전략을 매우 잘 학습했습니다!' :
  avgSuccess >= 60 ? '📚 AI가 기본적인 전략을 학습했습니다. 추가 학습으로 더 향상될 수 있습니다.' :
  avgSuccess >= 40 ? '📖 AI가 학습 중입니다. 500게임 이상 학습을 권장합니다.' :
  '🌱 아직 초기 학습 단계입니다. 더 많은 학습이 필요합니다.'}
...
${gen < 500 ? `
- ⏳ 최소 500게임 이상 학습을 권장합니다
- 🎯 현재 ${gen}게임 학습되었습니다
- 📈 더 많은 학습으로 성능이 크게 향상될 수 있습니다
` : avgSuccess < 70 ? `
- 📚 추가 학습으로 성능을 더 높일 수 있습니다
- 🧪 테스트 모드로 현재 실력을 확인해보세요
- 💾 현재 상태를 저장해두고 나중에 이어서 학습하세요
` : `
- ✅ AI가 충분히 학습되었습니다!
- 👁️ 관전 모드로 AI의 플레이를 감상해보세요
- 💾 학습 결과를 파일로 저장해 보관하세요
`}
...
const HelpModal = ({ onClose }) => (
  <div style={{
    ...
    <div style={{
      ...
      <section style={{ marginBottom: '20px' }}>
        <h3 style={{ color: '#2ecc71', fontSize: '1rem', marginBottom: '10px' }}>🎮 버튼 설명</h3>
        <div style={{ display: 'grid', gap: '8px' }}>
          {[
            { btn: '▶️ 학습', color: '#2ecc71', desc: 'AI가 시행착오를 통해 배웁니다' },
            { btn: '🧪 테스트', color: '#9b59b6', desc: '학습을 멈추고 현재 실력을 평가합니다' },
            { btn: '👁️ 관전', color: '#3498db', desc: 'AI 플레이를 실시간으로 관찰합니다' },
            { btn: '📋 리포트', color: '#f1c40f', desc: 'AI가 배운 전략을 분석합니다' },
            { btn: '📄 보고서', color: '#00f5ff', desc: '학습 결과를 파일로 저장합니다' },
            { btn: '🗑️ 초기화', color: '#e74c3c', desc: '학습을 리셋하고 처음부터 시작합니다' },
          ].map((m, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: `${m.color}15`, padding: '8px 12px', borderRadius: '6px' }}>
              <span style={{ background: m.color, padding: '4px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold', color: '#000', minWidth: '80px', textAlign: 'center' }}>{m.btn}</span>
      ...
      <section style={{ marginBottom: '20px' }}>
        <h3 style={{ color: '#f1c40f', fontSize: '1rem', marginBottom: '10px' }}>💡 사용 팁</h3>
        <div style={{ background: 'rgba(241,196,15,0.1)', padding: '12px', borderRadius: '8px', fontSize: '0.85rem', border: '1px solid rgba(241,196,15,0.3)' }}>
          <ol style={{ margin: 0, paddingLeft: '20px', lineHeight: 1.8 }}>
            ...
            <li>버튼에 <strong>마우스를 올리면</strong> 설명이 나타남</li>
          </ol>
        </div>
      </section>
...
const LearningReportModal = ({ agent, gen, avgScore, avgPieces, avgLines, successScore, onClose }) => {
  ...
  const lessons = [
    { name: '구멍 회피', pct: holesPct, icon: '🕳️', color: '#e74c3c', desc: '블록 사이에 빈 공간을 만들지 않습니다' },
    { name: '높이 관리', pct: heightPct, icon: '📏', color: '#3498db', desc: '블록을 가능한 낮게 쌓습니다' },
    { name: '평탄화', pct: bumpPct, icon: '🏔️', color: '#2ecc71', desc: '표면을 평평하게 유지합니다' },
    { name: '최대높이 제한', pct: maxHPct, icon: '⚠️', color: '#f1c40f', desc: '한쪽이 너무 높아지지 않게 합니다' },
  ].sort((a, b) => b.pct - a.pct);

  const strategies = [];
  if (holesPct > 25) strategies.push({ title: '구멍 최소화 전략', desc: '구멍이 생기지 않는 위치를 우선 선택', priority: holesPct });
  if (heightPct > 20) strategies.push({ title: '바닥 채우기 전략', desc: '낮은 위치에 블록을 배치', priority: heightPct });
  if (bumpPct > 20) strategies.push({ title: '평탄화 전략', desc: '높낮이 차이를 줄여 배치 용이하게', priority: bumpPct });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <section style={{ marginBottom: '16px' }}>
          <h3 style={{ color: '#2ecc71', fontSize: '0.9rem', marginBottom: '8px' }}>🎯 AI의 성공 전략</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {strategies.slice(0, 3).map((s, i) => (
              <div key={i} style={{
                background: 'rgba(46,204,113,0.1)', padding: '10px', borderRadius: '6px',
                border: '1px solid rgba(46,204,113,0.3)', display: 'flex', alignItems: 'center', gap: '8px'
              }}>
                <span style={{ fontSize: '1.2rem' }}>{['🥇', '🥈', '🥉'][i]}</span>
                <div>
                  <div style={{ fontWeight: 'bold', color: '#2ecc71', fontSize: '0.85rem' }}>{s.title}</div>
        ...
        <section style={{ marginBottom: '16px' }}>
          <h3 style={{ color: '#f1c40f', fontSize: '0.9rem', marginBottom: '8px' }}>💡 핵심 인사이트</h3>
          <div style={{ background: 'rgba(241,196,15,0.1)', padding: '10px', borderRadius: '6px', border: '1px solid rgba(241,196,15,0.3)', fontSize: '0.8rem', lineHeight: 1.5 }}>
            {gen < 100 ? (
              <p style={{ margin: 0 }}>아직 학습 초기입니다. 500게임 이상 학습하면 더 정확한 분석이 가능합니다.</p>
            ) : successScore >= 70 ? (
              <p style={{ margin: 0 }}>🎉 AI가 테트리스의 핵심 전략을 잘 학습했습니다! 특히 <strong>{lessons[0].name}</strong>을 가장 중요하게 여깁니다.</p>
            ) : (
              <p style={{ margin: 0 }}>📚 AI가 기본 전략을 배우는 중입니다. 더 많은 학습이 필요합니다.</p>
            )}
          </div>
        </section>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: '2px solid #e74c3c', boxShadow: '0 0 40px rgba(231,76,60,0.4)', textAlign: 'center', minWidth: '320px', maxWidth: '400px'
      }}>
        ...
        <div style={{ 
          ...
          <div style={{ fontSize: '0.75rem', opacity: 0.8, marginBottom: '8px' }}>삭제되는 항목:</div>
...
const GoalReachedModal = ({ successScore, gen, onContinue, onStop, onDownloadReport }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: 'linear-gradient(135deg, #1a1a2e, #0d0d1a)', borderRadius: '16px', padding: '28px',
        border: `2px solid #ffd700`, boxShadow: `0 0 40px rgba(255,215,0,0.4)`, textAlign: 'center', minWidth: '320px'
      }}>
...
const EfficiencyGraph = ({ efficiencyData, optimalPoint }) => {
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.4)', borderRadius: '8px', padding: '10px', marginTop: '8px' }}>
      <div style={{ fontSize: '0.7rem', fontWeight: 'bold', marginBottom: '6px' }}>📈 학습량 vs 성공률</div>
      ...
      {optimalPoint && (
        <div style={{ marginTop: '6px', padding: '5px 8px', background: 'rgba(255,215,0,0.15)', borderRadius: '4px', fontSize: '0.65rem' }}>
    ...
    <div style={{ background: 'rgba(0,0,0,0.4)', borderRadius: '6px', padding: '8px', marginTop: '8px' }}>
      ...
      {insights.map((item, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
          ...
          <span style={{ fontSize: '0.5rem', opacity: 0.6, width: '35px' }}>{item.text}</span>
        </div>
      ))}
    </div>
  );
