...
\fancyhead[R]{\small\color{textmedium}v1.2.1.4}
\fancyfoot[C]{\thepage}
...
\begin{center}
    ...
    {\Large 사용설명서}\\[6pt]
    {\large\color{textmedium} v1.2.1.4}\\[20pt]
\end{center}
...
퀵가이드 & → & \colorbox{accentyellow}{\textbf{사용설명서}} & → & 이론해설서 & → & 활용가이드 & → & 수업지도안 \\
...
\hline
\textbf{1} & \textcolor{primarygreen}{\textbf{📊 데이터 생성}} & 왼쪽 패널에서 \texttt{[✨ 데이터 생성]} 버튼 클릭 \\
\hline
\textbf{2} & \textcolor{primaryblue}{\textbf{⚡ 학습 실행}} & 오른쪽 패널에서 \texttt{[⚡ 학습 실행]} 버튼 클릭 \\
\hline
\textbf{3} & \textcolor{primaryblue}{\textbf{🔮 추론 모드}} & 상단 \texttt{[🔮 추론 모드]} 탭 전환 후 슬라이더 조작 \\
\hline
\end{tabular}
\end{center}
...
사과의 \textbf{🍯 당도}와 \textbf{⚖️ 무게}를 \textbf{⭐ 품질 점수}로 통합하여 \textbf{💰 가격}을 예측하는 과정을 시각적으로 체험할 수 있습니다.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{📽️ 학습 과정 재생}: 애니메이션으로 학습 과정 관찰
\end{itemize}
...
⚖️ 무게 점수 & $(\text{무게} - 150) / 200 \times 50$ & 150\textasciitilde350g → 0\textasciitilde50점 \\
\midrule
...
\begin{itemize}
    ...
    \item ⚖️ 무게 250g: $(250 - 150) / 200 \times 50 = 25$점
...
\midrule
학습률 & 0.01 & 한 번에 얼마나 크게 움직일지 결정. 너무 크면 불안정 ⚠️ \\
에포크 & 500 & 전체 데이터를 몇 번 반복 학습할지 🔄 \\
\bottomrule
\end{tabular}
\end{center}
...
$0.5 \sim 0.8$ & ⚠️ 보통 & 어느 정도 설명하지만 개선 여지 있음 \\
$\leq 0.5$ & ❌ 부족 & 모델 개선 필요 \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
⚖️ 가중치 (w) & 기울기. X가 1 증가할 때 Y가 얼마나 변하는지 \\
\midrule
...
\midrule
⬇️ 경사 하강법 & 손실을 줄이는 방향으로 가중치를 조금씩 조정하는 방법 \\
\midrule
...
\textbf{v1.2.1.4} & \textbf{2026-02-01} & \textbf{19:10} & \textbf{패치} & \textbf{이모지 정상 표시, 색상 변경} & \textbf{Claude} \\
\bottomrule
\end{tabular}
\end{center}
...
\vfill
\begin{center}
\textit{🍎 사과 가격 예측 선형 회귀 시뮬레이터 사용설명서 v1.2.1.4}
\end{center}
