% 사과 가격 예측 선형 회귀 시뮬레이터 이론해설서 v1.5.1.0
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[R]{\small\color{lightbrown}선형 회귀 이론해설서 v1.5.1.0}
\fancyfoot[C]{\thepage}
...
\begin{tcolorbox}[colback=primary, colframe=primary, arc=15pt, width=4cm]
  \centering\large\bfseries\color{white} v1.5.0.0
\end{tcolorbox}
...
\addcontentsline{toc}{section}{문서 사용방법}

\subsection*{이 문서의 목적}

이 문서는 \textbf{``사과 가격 예측 선형 회귀 시뮬레이터''}의 이론적 배경을 설명합니다. 시뮬레이터를 사용하기 전에 읽거나, 사용하면서 참고하세요.

\subsection*{대상 독자}

\begin{itemize}
  \item 초등 예비교사 (컴퓨팅사고 교육)
  \item AI/머신러닝 입문자
  \item 선형 회귀의 원리를 시각적으로 이해하고 싶은 분
\end{itemize}
...
\hline
2 & \emoji{red-apple} 시뮬레이터 실행 & 데이터 생성 → 학습 → 추론 체험 \\
\hline
3 & \emoji{books} 본문 읽기 & 궁금한 개념을 찾아서 읽기 \\
\hline
4 & \emoji{video-game} 시뮬레이터 재실험 & 파라미터 변경하며 이론 확인 \\
\hline
\end{tabular}
\end{center}
...
\begin{tcolorbox}[colback=cream, colframe=primary, boxrule=1pt, arc=3mm]
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|p{6cm}|p{4cm}|}
\hline
\rowcolor{tableheader}
\textbf{\#} & \textbf{문서명} & \textbf{용도} \\
\hline
1 & 시뮬레이터\_v1.2.4.1.html & \textbf{실습용} - 먼저 실행해보세요! \\
\hline
2 & 퀵가이드\_v1.2.4.1.md & 시뮬레이터 조작법 (2페이지) \\
\hline
3 & 개발일지\_v1.2.4.1.md & 기술 상세 (개발자용) \\
\hline
4 & \textbf{이론해설서 (본 문서)} & 이론 배경 설명 \\
\hline
\end{tabular}
\end{center}
\end{tcolorbox}
...
이 문서를 읽으면서 시뮬레이터를 \textbf{동시에 실행}해두면 이해가 훨씬 쉽습니다.

예: ``경사 하강법'' 섹션을 읽을 때 → 시뮬레이터에서 \emoji{play-button} 재생 버튼을 눌러 회귀선이 움직이는 모습을 확인하세요!
\end{tcolorbox}
...
\formulawithexp{$y = w \cdot x + b$}{예측 가격 = (품질의 영향력 $\times$ 품질 점수) + 기본 가격}
...
\hline
당도점수 & $\displaystyle\frac{\text{당도} - 10}{8} \times 50$ & 10\textasciitilde18 Brix → 0\textasciitilde50점 \\
\hline
무게점수 & $\displaystyle\frac{\text{무게} - 150}{200} \times 50$ & 150\textasciitilde350g → 0\textasciitilde50점 \\
\hline
품질 (총합) & $\text{당도점수} + \text{무게점수}$ & 0\textasciitilde100점 \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item 당도점수 $= \dfrac{14 - 10}{8} \times 50 = $ \textbf{25점}
  \item 무게점수 $= \dfrac{250 - 150}{200} \times 50 = $ \textbf{25점}
...
\formulawithexp{$\displaystyle z = \frac{x - \mu}{\sigma}$ \quad (정규화된 값 $=$ $\dfrac{\text{원래 값} - \text{평균}}{\text{표준편차}}$)}{``평균에서 얼마나 떨어졌는가''를 표준 단위로 표현}
...
\formulawithexp{$\displaystyle \text{MSE} = \frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{y}_i)^2$}{``오차들을 제곱해서 평균 낸 값'' - 큰 오차에 더 큰 페널티!}

MSE는 오차를 제곱하여 평균을 구합니다. \textbf{큰 오차에 더 큰 페널티}를 부여합니다.

\subsection{오차(잔차)의 의미}

\formulawithexp{$\text{오차} = y_i - \hat{y}_i$ \quad (실제 가격 $-$ 예측 가격)}{``AI가 틀린 정도'' - 양수면 과소예측, 음수면 과대예측}
...
\formulawithexp{$w_{\text{new}} = w_{\text{old}} - \eta \cdot \nabla L$}{``새 가중치 = 기존 위치 $-$ (학습률 $\times$ 경사방향)''}
...
\formulawithexp{$\displaystyle R^2 = 1 - \frac{SS_{\text{res}}}{SS_{\text{tot}}} = 1 - \frac{\sum(y_i - \hat{y}_i)^2}{\sum(y_i - \bar{y})^2}$}{``모델이 데이터의 변동성을 얼마나 잘 설명하는가'' (1에 가까울수록 좋음)}
...
\hline
2 & 가중치 초기화 & $w=0$, $b=0$으로 시작 \\
\hline
3 & 예측 & $\text{가격} = w \times \text{품질} + b$ 계산 \\
\hline
...
\hline
\textbf{v1.5.0.0} & \textbf{2026-02-02} & \textbf{15:45} & \textbf{마이너} & \textbf{문서 사용방법 섹션 추가 (목적, 대상, 학습순서, 관련문서)} & \textbf{Claude} \\
\hline
\textbf{v1.5.1.0} & \textbf{2026-02-02} & \textbf{16:00} & \textbf{리비전} & \textbf{모든 수식을 LaTeX 수학 수식으로 변경 (amsmath)} & \textbf{Claude} \\
\hline
\end{tabular}
\end{center}
